...
from flcore.servers.serversslavg import FedSSLAvg
from flcore.servers.serverslavg import FedSLAvg
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif model_str == "bcnn":  # non-convex
            if "MNIST" in args.dataset:
                args.model = FedAvgBCNN(
                    in_features=1, num_classes=args.num_classes, dim=1024).to(args.device)
            elif "Cifar10" in args.dataset:
                args.model = FedAvgBCNN(
                    in_features=3, num_classes=args.num_classes, dim=1600).to(args.device)
        ...
        elif args.algorithm == "FedSSLAvg":
            server = FedSSLAvg(args, i)
        elif args.algorithm == "FedSLAvg":
            server = FedSLAvg(args, i)
