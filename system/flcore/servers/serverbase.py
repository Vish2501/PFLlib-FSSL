...
class Server(object):
    ...
    #         for key in global_state_dict:
    #             global_state_dict[key] += w * client_state_dict[key]

    #     # Load the updated state dict into the global model
    ...
    #     # Zero out all parameters in the global model
    #     for key in global_state_dict.keys():
    #         global_state_dict[key].zero_()

    #     # Accumulate parameters from all clients based on their weights
    #     for w, client_model in zip(self.uploaded_weights, self.uploaded_models):
    #         client_state_dict = client_model.state_dict()
    #         for key in global_state_dict.keys():
    #             global_state_dict[key] += w * client_state_dict[key]

    #     # Load the accumulated and averaged parameters back into the global model
    #     self.global_model.load_state_dict(global_state_dict)
    ...
    def test_metrics(self):
        ...
        if self.algorithm == 'FedSSLAvg':

            for c in self.clients:
                ct, ns, auc = c.bnn_test_metrics()
                tot_correct.append(ct*1.0)
                tot_auc.append(auc*ns)
                num_samples.append(ns)

        elif self.algorithm == 'FedSLAvg':
            for c in self.clients:
                ct, ns, auc = c.bnn_test_metrics()
                tot_correct.append(ct * 1.0)
                tot_auc.append(auc * ns)
